 ...
 */
function createIcon(iconPath: string): string {
  return `<svg class="octicon octicon-icon mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="${iconPath}"></path></svg>`;
}
  ...
  while (i < lines.length) {
    ...
    if (alertMatch) {
      ...
      if (contentLines.length > 0) {
        const icon = createIcon(config.iconPath);
        const content = contentLines.join('\n');

        result.push(
          `<div class="markdown-alert ${config.className}" dir="auto">` +
...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (app: JupyterFrontEnd, markdownParser: IMarkdownParser) => {
