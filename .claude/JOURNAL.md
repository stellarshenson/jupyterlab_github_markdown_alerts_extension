...
11. **Task - Prepare release 1.0.5**: Created release tag and updated release documentation<br>
    **Result**: Created and pushed RELEASE_1.0.5 tag to remote repository. Updated CHANGELOG.md with comprehensive release notes for version 1.0.5 including features (alert rendering, theme support, icons, settings), documentation updates (screenshot, badges, examples), bug fixes (icon rendering, markdown parsing, CSS styling, code block detection), and infrastructure changes (workflow updates). CHANGELOG follows conventional commit format with categorized sections.
